...
public class CityControllerTest {
	...
	public void test() throws Exception {
		{
			// Select
			List<City> cities = restTemplate.exchange("http://localhost:" + port, HttpMethod.GET, HttpEntity.EMPTY,
		...
		{
			// Update
			List<City> cities = restTemplate.exchange("http://localhost:" + port + "/update?name=Kyoto", HttpMethod.GET,
					HttpEntity.EMPTY, new ParameterizedTypeReference<List<City>>() {
					}).getBody();
			assertThat(cities.size(), is(3));
			assertThat(cities.get(0).id, is(1));
